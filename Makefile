# Project Setup
PROJECT_NAME := configuration-gcp-gke-workload-identity
...
ifndef UPTEST_CLOUD_CREDENTIALS
	@$(INFO) Please export UPTEST_CLOUD_CREDENTIALS, e.g. \`export UPTEST_CLOUD_CREDENTIALS=\$\(cat \~/gcp.json\)\`
	@$(FAIL)
endif
...
uptest: $(UPTEST) $(KUBECTL) $(KUTTL)
	...
	@KUBECTL=$(KUBECTL) KUTTL=$(KUTTL) CROSSPLANE_NAMESPACE=$(CROSSPLANE_NAMESPACE) $(UPTEST) e2e examples/network-xr.yaml,examples/gke-xr.yaml,examples/workload-identity-xr.yaml --data-source="${UPTEST_DATASOURCE_PATH}" --setup-script=test/setup.sh --default-timeout=2400 $(SKIP_DELETE) || $(FAIL)
...
render: ## Crossplane render
	crossplane beta render examples/workload-identity-xr.yaml apis/composition.yaml examples/functions.yaml -r
